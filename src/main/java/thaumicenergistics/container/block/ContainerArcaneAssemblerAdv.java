 ...
 * @author Alex811
 */
public class ContainerArcaneAssemblerAdv extends ContainerBase {
    protected TileArcaneAssemblerAdv TE;

    public ContainerArcaneAssemblerAdv(EntityPlayer player, TileArcaneAssemblerAdv TE) {
        super(player);
        this.TE = TE;
    ...
    public TileArcaneAssemblerAdv getTE() {
        return TE;
    }
    ...
    private class KnowledgeCoreSlotListener implements IContainerListener{
        ...
        public void sendSlotContents(Container containerToSend, int slotInd, ItemStack stack) {
            if(slotInd == 0 && opened && ForgeUtil.isServer()) {
                IGridNode node = ContainerArcaneAssemblerAdv.this.TE.getActionableNode();
                ContainerArcaneAssemblerAdv.this.playCoreSound(ContainerArcaneAssemblerAdv.this.player);
                node.getGrid().postEvent(new MENetworkCraftingPatternChange(ContainerArcaneAssemblerAdv.this.TE, node)); // update ME system available patterns
            }
            opened = true;
        }
