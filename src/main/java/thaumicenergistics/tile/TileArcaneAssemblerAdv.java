...
import thaumicenergistics.network.packets.PacketAssemblerAdvGUIUpdate;
 ...
 * @author Alex811
 */
public class TileArcaneAssemblerAdv extends TileArcaneAssembler {
    ...
    public TileArcaneAssemblerAdv() {
        super();
        ItemStack assemblerItem = ThEApi.instance().blocks().arcaneAssemblerAdv().maybeStack(1).orElseThrow(RuntimeException::new);
        this.coreInv = new ThEKnowledgeCoreInventory("cores", 1, 1, assemblerItem);
        this.upgradeInv = new ThEUpgradeInventory("upgrades", 8, 1, assemblerItem);
    ...
    public ModGUIs getGUI() {
        return ModGUIs.ARCANE_ASSEMBLER_ADV;
    }
    ...
    public TickingRequest getTickingRequest(@Nonnull IGridNode node) {
        return new TickingRequest(ThEApi.instance().config().tickTimeArcaneAssemblerAdvMin(), ThEApi.instance().config().tickTimeArcaneAssemblerAdvMax(), !this.isBusy(), false);
    }
        ...
        if(this.craftingInv.getStackInSlot(0).isEmpty()) {
            ...
            if(this.progress >= 100){
                ...
                if(Stream.of(stack, inventory, this.src).anyMatch(Objects::isNull)){
                    this.progress -= getStep();
                    ThELog.trace("Advanced Arcane Assembler @ (" + this.getPos().getX() + ", " + this.getPos().getY() + ", " + this.getPos().getZ() + "): ME system not ready for crafting yet, retrying...");
    ...
    protected void notifySubs(){ // update client side, to show details in the GUI
        this.notifySubs(player -> PacketHandler.sendToPlayer((EntityPlayerMP) player, new PacketAssemblerAdvGUIUpdate(this)));
    }
    ...
    public void withInfoText(Consumer<String> consumer, Function<IThELangKey, String> localizationMapper){
        if(this.isActive()){
            if(this.hasJob()){
                if(this.isCrafting()){
                    consumer.accept(localizationMapper.apply(ThEApi.instance().lang().arcaneAssemblerAdvBusy()));
                    consumer.accept(localizationMapper.apply(ThEApi.instance().lang().arcaneAssemblerAdvProgress()) + " " + this.getProgress() + "%");
                }else{
                    consumer.accept(localizationMapper.apply(ThEApi.instance().lang().arcaneAssemblerAdvPrep()));
                    if(this.isMissingAspect())
                        consumer.accept(localizationMapper.apply(ThEApi.instance().lang().arcaneAssemblerAdvNoAspect()));
                    if(!this.getHasEnoughVis())
                        consumer.accept(localizationMapper.apply(ThEApi.instance().lang().arcaneAssemblerAdvNoVis()));
                }
            }else
                consumer.accept(localizationMapper.apply(ThEApi.instance().lang().arcaneAssemblerAdvIdle()));
        }
    }
    ...
    protected float getWorldVis() {
        return ThEApi.instance().items().upgradeArcane().maybeStack(1).map(visRangeUpgrade -> {
            ...
            if(this.upgradeInv.getUpgrades(visRangeUpgrade) > 0){
                vis += AuraHelper.getVis(this.getWorld(), this.getPos().add(-64, 0, -64));
                vis += AuraHelper.getVis(this.getWorld(), this.getPos().add(-64, 0, 0));
                vis += AuraHelper.getVis(this.getWorld(), this.getPos().add(-64, 0, 64));

                vis += AuraHelper.getVis(this.getWorld(), this.getPos().add(0, 0, -64));
                vis += AuraHelper.getVis(this.getWorld(), this.getPos().add(0, 0, 64));
