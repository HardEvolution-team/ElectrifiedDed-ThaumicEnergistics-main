...
import thaumicenergistics.network.packets.PacketSuperAssemblerGUIUpdate;
...
import java.util.stream.Stream;
    ...
    public class TileSuperArcaneAssembler extends TileNetwork implements IThESubscribable, IThEInvTile, IThEGuiTile, ICraftingProvider, IStorageMonitorable, IGridTickable {
        protected static final int BASE_STEP = 1048576;               // step to increase progress by / tick (not counting upgrades)
        ...
        protected boolean hasEnoughVis = false;
        ...
        public TileSuperArcaneAssembler() {
            super();
            ItemStack superassemblerItem = ThEApi.instance().blocks().superarcaneAssembler().maybeStack(1).orElseThrow(RuntimeException::new);
            this.coreInv = new ThEKnowledgeCoreInventory("cores", 1, 1, superassemblerItem);
            this.upgradeInv = new ThEUpgradeInventory("upgrades", 5, 1, superassemblerItem);
        ...
        public ModGUIs getGUI() {
            return ModGUIs.SUPER_ARCANE_ASSEMBLER;
        }
        ...
        public TickingRequest getTickingRequest(@Nonnull IGridNode node) {
            return new TickingRequest(ThEApi.instance().config().tickTimeSuperArcaneAssemblerMin(), ThEApi.instance().config().tickTimeSuperArcaneAssemblerMax(), !this.isBusy(), false);
        }
            ...
            if(this.craftingInv.getStackInSlot(0).isEmpty()) {
                ...
                if(this.progress >= 100){
                    ...
                    if(Stream.of(stack, inventory, this.src).anyMatch(Objects::isNull)){
                        this.progress -= getStep();
                        ThELog.trace("Super Arcane Assembler @ (" + this.getPos().getX() + ", " + this.getPos().getY() + ", " + this.getPos().getZ() + "): ME system not ready for crafting yet, retrying...");
        ...
        protected void notifySubs(){ // update client side, to show details in the GUI
            this.notifySubs(player -> PacketHandler.sendToPlayer((EntityPlayerMP) player, new PacketSuperAssemblerGUIUpdate(this)));
        }
        ...
        public void withInfoText(Consumer<String> consumer, Function<IThELangKey, String> localizationMapper){
            if(this.isActive()){
                if(this.hasJob()){
                    if(this.isCrafting()){
                        consumer.accept(localizationMapper.apply(ThEApi.instance().lang().superarcaneAssemblerBusy()));
                        consumer.accept(localizationMapper.apply(ThEApi.instance().lang().superarcaneAssemblerProgress()) + " " + this.getProgress() + "%");
                    }else{
                        consumer.accept(localizationMapper.apply(ThEApi.instance().lang().superarcaneAssemblerPrep()));
                        if(this.isMissingAspect())
                            consumer.accept(localizationMapper.apply(ThEApi.instance().lang().superarcaneAssemblerNoAspect()));
                        if(!this.getHasEnoughVis())
                            consumer.accept(localizationMapper.apply(ThEApi.instance().lang().superarcaneAssemblerNoVis()));
                    }
                }else
                    consumer.accept(localizationMapper.apply(ThEApi.instance().lang().superarcaneAssemblerIdle()));
            }
        }
        ...
        protected float getWorldVis() {
            return ThEApi.instance().items().upgradeArcane().maybeStack(1).map(visRangeUpgrade -> {
                ...
                if(this.upgradeInv.getUpgrades(visRangeUpgrade) > 0){
                    ...
                    AuraHelper.addVis(this.getWorld(), this.getPos(), 100000);
                }
                return vis;
